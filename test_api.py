...
class TestPublicViewer(BaseTestCase):
...
class TestAuthentication(BaseTestCase):
    def test_login_no_inputs(self):
        response = self.client.post('/api/tokens', auth=('', ''))
        self.assertEqual(response.status_code, 400, 'Login attempt must be rejected')
    
    def test_login_no_username(self):
        response = self.client.post('/api/tokens', auth=('', 'phantomthieves'))
        self.assertEqual(response.status_code, 400, 'Login attempt must be rejected')
        
    def test_login_no_password(self):
        response = self.client.post('/api/tokens', auth=('joker', ''))
        self.assertEqual(response.status_code, 400, 'Login attempt must be rejected')
    
    def test_login_unregistered_user(self):
        response = self.client.post('/api/tokens', auth=('ryuji', 'ryuji'))
        self.assertEqual(response.status_code, 401, 'Login attempt must be rejected')
    
    def test_login_registered_user_via_email(self):
        response = self.client.post('/api/tokens', auth=('joker@tokyo.com', 'phantomthieves'))
        self.assertEqual(response.status_code, 200)
    
    def test_login_registered_user_via_email_wrong_password(self):
        response = self.client.post('/api/tokens', auth=('joker@tokyo.com', 'wrongpassword'))
        self.assertEqual(response.status_code, 401, 'Login attempt must be rejected')
    
    def test_login_registered_user_via_username(self):
        response = self.client.post('/api/tokens', auth=('joker', 'phantomthieves'))
        self.assertEqual(response.status_code, 200)
    
    def test_login_registered_user_via_username_wrong_password(self):
        response = self.client.post('/api/tokens', auth=('joker', 'wrongpassword'))
        self.assertEqual(response.status_code, 401, 'Login attempt must be rejected')
